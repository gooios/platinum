...
// Copyright (c) 2013-2014 The Platinum developers
...
// Platinum: get network Gh/s estimate
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    // platinum: if user OS account compromised prevent trivial sendmoney commands
    if (params.size() > 2)
...
// platinum: get information of sync-checkpoint
...
// platinum: reserve balance from being staked for network protection
...
// platinum: check wallet integrity
...
void ThreadRPCServer2(void* parg)
{
    ...
    ip::tcp::endpoint endpoint(bindAddress, GetArg("-rpcport", 10126));
...
Object CallRPC(const string& strMethod, const Array& params)
{
    ...
    SSLIOStreamDevice d(sslStream, fUseSSL);
    iostreams::stream<SSLIOStreamDevice> stream(d);
    if (!d.connect(GetArg("-rpcconnect", "127.0.0.1"), GetArg("-rpcport", CBigNum("10126").ToString().c_str())))
